...
//GC within a file of records
//GC to compare the items the function uses the compare function pointer
...
//GC if the file is too small, it gets enlarged returns the number of bytes written
int binaryFileWrite(int fd, void* item, int item_size, int pos);  
//GC writes a record of size item_size in position pos in the file
//GC if the file is too small, it gets enlarged and returns the number of bytes read or a negative number if error
